...
## Trust and Capability Mapping

A complete threat model should also document:

- who must be trusted for the system to remain safe
- which actors can directly move funds or change security posture
- which controls are enforced by code versus by process
- which compromise sets cause catastrophic loss versus operational disruption

For a practical method, see [Trust Boundaries and Capability Inventories](./trust-boundaries-and-capability-inventories).
